...
/// Column count for a dashboard `GridView`/`Wrap` grid, stepped down as the
/// screen narrows so a fixed column count doesn't squeeze cards (or charts)
/// into unreadably thin slices on a phone.
int responsiveGridColumns(
  ...
  final width = MediaQuery.sizeOf(context).width;
  if (width < kBreakpointMedium) return phone;
  if (width < kBreakpointLarge) return tablet;
  return desktop;
}

/// Clamps a dialog's desktop-designed width so it never exceeds ~92% of a
/// narrow screen (where it would otherwise render wider than the phone and
/// get clipped), while leaving wider screens exactly as designed.
double responsiveDialogWidth(BuildContext context, double desktopWidth) {
  final maxAllowed = MediaQuery.sizeOf(context).width * 0.92;
  return desktopWidth > maxAllowed ? maxAllowed : desktopWidth;
}
