...
import 'energydetailscurrentcard_model.dart';
export 'energydetailscurrentcard_model.dart';

class EnergydetailscurrentcardWidget extends StatefulWidget {
  final Map<String, dynamic> currentData;
  const EnergydetailscurrentcardWidget({super.key, required this.currentData});

  @override
  State<EnergydetailscurrentcardWidget> createState() =>
      _EnergydetailscurrentcardWidgetState();
}

class _EnergydetailscurrentcardWidgetState
    extends State<EnergydetailscurrentcardWidget>
    with TickerProviderStateMixin {
  late EnergydetailscurrentcardModel _model;
  ...
  // count-up controllers per value key
...
// ─────────────────────────────────────────────────────────────
// FIX 1 — initState: initialise controllers BEFORE postFrameCallback
// Replace your existing initState animation setup with this pattern
// (change the keys list to match each file's data keys)
// ─────────────────────────────────────────────────────────────
  ...
  void initState() {
    ...
    _model = createModel(context,
        () => EnergydetailscurrentcardModel()); // change model per file
    ...
    for (final key in ['IA', 'IB', 'IC', 'Iavg']) {
  ...
  @override
  void didUpdateWidget(covariant EnergydetailscurrentcardWidget old) {
  ...
  void _triggerCountUp() {
    for (final key in ['IA', 'IB', 'IC', 'Iavg']) {
      final raw = widget.currentData[key];
  ...
  String valueOrNA(dynamic v) {
    ...
    if (v is String && v.toUpperCase() == 'NA') return 'No Data';
    return 'No Data';
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        Expanded(
                          ...
                          child: Row(children: [
                            Expanded(
                                child: _buildCompactCell(ctx, 'Current A', 'IA',
                                    'A', accent, Icons.electric_bolt_rounded)),
                            const SizedBox(width: 6),
                            Expanded(
                                child: _buildCompactCell(ctx, 'Current B', 'IB',
                                    'A', accent, Icons.electric_bolt_rounded)),
                            const SizedBox(width: 6),
                            Expanded(
                                child: _buildCompactCell(ctx, 'Current C', 'IC',
                                    'A', accent, Icons.electric_bolt_rounded)),
                          ]),
                        ),
                        ...
                        Expanded(
                          flex: 5,
                          child: _buildFeaturedCell(ctx, 'Current Average',
                              'Iavg', 'A', accent, Icons.electric_bolt_rounded),
                        ),
                      ],
                    ),
                  ),
