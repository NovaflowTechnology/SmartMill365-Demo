...
import 'energydetailsenergycard_model.dart';
export 'energydetailsenergycard_model.dart';

class EnergydetailsenergycardWidget extends StatefulWidget {
  final Map<String, dynamic> energyData;
  const EnergydetailsenergycardWidget({super.key, required this.energyData});

  @override
  State<EnergydetailsenergycardWidget> createState() =>
      _EnergydetailsenergycardWidgetState();
}

class _EnergydetailsenergycardWidgetState
    extends State<EnergydetailsenergycardWidget> with TickerProviderStateMixin {
  late EnergydetailsenergycardModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => EnergydetailsenergycardModel());
  ...
  @override
  void didUpdateWidget(covariant EnergydetailsenergycardWidget old) {
  ...
  void _triggerCountUp() {
    for (final key in ['Edel', 'Erec']) {
      final raw = widget.energyData[key];
  ...
  Widget _countUpText(String key, String suffix, TextStyle style) =>
      AnimatedBuilder(
        ...
        builder: (_, __) {
          ...
          if (!widget.energyData.containsKey(key))
            return Text('No Data', style: style);
          return Text('${addCommas(val.round().toString())} $suffix',
              style: style);
        },
      );

  // Energy card uses featured cells for BOTH rows (no compact cells)
