...
class MachineOEEComparisonWidget extends StatefulWidget {
  final Map<String, dynamic>? data;
  final String? selectedShift;

  const MachineOEEComparisonWidget({
    super.key,
    this.data,
    this.selectedShift,
  });
  ...
  @override
  State<MachineOEEComparisonWidget> createState() => _MachineOEEComparisonWidgetState();
}

class _MachineOEEComparisonWidgetState extends State<MachineOEEComparisonWidget> {
  List<Map<String, dynamic>> _processData() {
    ...
    for (var item in apiData) {
      ...
      if (widget.selectedShift == null || widget.selectedShift == 'All') {
        processed.add({
          'machine': machine,
          'morning': (item['Morning_OEE'] ?? 0).toDouble(),
          'night': (item['Night_OEE'] ?? 0).toDouble(),
        });
      } else if (widget.selectedShift?.toLowerCase() == 'morning') {
        processed.add({
          'machine': machine,
          'morning': (item['Morning_OEE'] ?? item['OEE'] ?? 0).toDouble(),
          'night': 0.0,
        });
        ...
        processed.add({
          'machine': machine,
          'morning': 0.0,
          'night': (item['Night_OEE'] ?? item['OEE'] ?? 0).toDouble(),
        });
      }
    }
  ...
  Widget build(BuildContext context) {
    List<Map<String, dynamic>> oeeData = _processData();
    ...
    double minChartWidth = oeeData.length * (barWidth + groupSpacing);
              ...
              child: SizedBox(
                ...
                child: BarChart(
                  BarChartData(
                    ...
                    barTouchData: BarTouchData(
                      ...
                      touchTooltipData: BarTouchTooltipData(
                        ...
                        getTooltipItem: (group, groupIndex, rod, rodIndex) {
                          if (groupIndex >= oeeData.length) return null;
                          if (rod.toY == 0) return null;

                          String shift = rodIndex == 0 ? 'Morning' : 'Night';
                          return BarTooltipItem(
                            '${oeeData[groupIndex]['machine']}\n$shift OEE: ${rod.toY.toStringAsFixed(1)}%',
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            if (value.toInt() < oeeData.length) {
                              return Padding(
                                padding: const EdgeInsets.only(top: 8),
                                child: RotatedBox(
                                  quarterTurns: 3,
                                  child: Text(
                                    oeeData[value.toInt()]['machine'],
                    ...
                    barGroups: oeeData.asMap().entries.map((entry) {
                      int index = entry.key;
