...
class EditAbnormalReasonDialog extends StatefulWidget {
  const EditAbnormalReasonDialog({
    super.key,
    required this.id,
    required this.initialReasonNo,
    required this.initialReasonDescription,
    required this.initialType,
    required this.initialProductionArea,
    required this.initialEquipmentModel,
    required this.userId,
    required this.onEdited,
  });

  final String id;
  final String initialReasonNo;
  final String initialReasonDescription;
  final String initialType;
  final List<String> initialProductionArea;
  final List<String> initialEquipmentModel;
  final String userId;
  final VoidCallback onEdited;

  @override
  State<EditAbnormalReasonDialog> createState() => _EditAbnormalReasonDialogState();
}

class _EditAbnormalReasonDialogState extends State<EditAbnormalReasonDialog> {
  final _formKey = GlobalKey<FormState>();
  late final TextEditingController _reasonNoCtrl;
  late final TextEditingController _reasonDescCtrl;
  ...
  void initState() {
    super.initState();
    _reasonNoCtrl = TextEditingController(text: widget.initialReasonNo);
    _reasonDescCtrl = TextEditingController(text: widget.initialReasonDescription);
    _selectedType = widget.initialType.isNotEmpty ? widget.initialType : null;
    _selectedAreas = List<String>.from(widget.initialProductionArea);
    _selectedEquipment = List<String>.from(widget.initialEquipmentModel);
    _fetchAreas();
    _fetchEquipment();
  }
  ...
  Future<bool> _submit() async {
    ...
    try {
      final res = await http.put(
        Uri.parse('${AppConfig.dataApiBaseSafe}/abnormalReasons/${widget.id}'),
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: isLight
                  ...
                  : const Color(0xFF31ECFC).withOpacity(0.3)),
        ),
        child: Row(
  ...
  Widget build(BuildContext context) {
    ...
    return CustomDialog(
      ...
      title: 'Edit Abnormal Reason',
      subtitle: 'Update the abnormal reason details.',
      requiredNote: true,
      submitLabel: 'SAVE',
      onSubmit: _submit,
      sections: [
        CustomDialogSection(
          ...
          subtitle: 'Modify the abnormal reason information.',
          children: [
            TextFormField(
