...
// Columns map 1:1 to mySQL table Overall_hourly_energy_consumption
// (filtered by device_id); Display Name & Status come from Master
// Facilities joined on Device ID. Device set (Plant/Prod Area/Device
// filters) is every Master Facilities record with a Device ID, not just
// ones tagged Digital Power Meter.
...
class EnergyDataLoggerView extends StatefulWidget {
  const EnergyDataLoggerView({super.key});

  @override
  State<EnergyDataLoggerView> createState() => _EnergyDataLoggerViewState();
}

class _EnergyDataLoggerViewState extends State<EnergyDataLoggerView> {
  // ── Filter state ──────────────────────────────────────────────────────────
  ...
  // Raw hourly readings from mySQL Overall_hourly_energy_consumption.
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // All registered devices with a Device ID — not just Digital Power
      // Meters — so Plant/Prod Area/Device filters mirror the full Master
      // Facilities list.
      final meters = all.where((f) => f.meterId.isNotEmpty && f.meterId != '-').toList();
  ...
  // Filter server-side so LIMIT applies per device/range, not across all meters.
  String? _dataLoggerDeviceIds() {
    final ids = _filtered
        .map((f) => f.meterId)
        .where((id) => id.isNotEmpty)
        .toList();
    if (ids.isEmpty) return null;
    if (_device != 'All Devices') return ids.first;
    if (_allMeters.isNotEmpty && ids.length >= _allMeters.length) return null;
    return ids.join(',');
  }

  // Fetch hourly readings from the backend (mySQL table
  // Overall_hourly_energy_consumption, filtered by device_id server-side).
  ...
  Future<List<Map<String, dynamic>>> _fetchHourlyRows() async {
    try {
      ...
      final deviceIds = _dataLoggerDeviceIds();
      if (deviceIds != null) params['device_id'] = deviceIds;
  ...
  // ── Table rows ─────────────────────────────────────────────────────────────
  // Real rows: mySQL Overall_hourly_energy_consumption joined to Master
  ...
  List<TableRecord> get _tableRows {
    ...
    if (_hourlyRows.isNotEmpty) {
      ...
      for (final r in _hourlyRows) {
        ...
        if (f == null) continue; // only meters registered in Master Facilities
        ...
        final kwh = numOf(r['energy_consumption_kWh']);
        ...
        rows.add(<String, dynamic>{
          ...
          'meterName': f.meterName.isNotEmpty ? f.meterName : deviceId,
          'meterId': deviceId,
          'status': f.status.isNotEmpty ? f.status : 'Inactive',
          // All values shown with 2 decimal places (client request).
          'kwh': _numStr(r['energy_consumption_kWh'], dp: 2),
  ...
  Future<void> _exportCsv() async {
    ...
    try {
      ...
      await csv_saver.saveExcelBytes(
        bytes: bytes,
        fileName: 'EnergyDataLogger_$ts.csv',
      );
      if (!mounted) return;
      ...
      children: [
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Dashboard / Energy Data Logger', style: GoogleFonts.poppins(fontSize: 11, color: Colors.grey.shade400)),
              ...
              Text('All devices — sourced from Master Facilities', style: GoogleFonts.poppins(fontSize: 11, color: t.secondaryText)),
            ],
          ),
        ),
...
class _EditDialogState extends State<_EditDialog> {
  ...
  void initState() {
    ...
    _regDate = _EnergyDataLoggerViewState._parseDate(widget.row['registrationDate']?.toString() ?? '');
    _loadDropdowns();
  }
